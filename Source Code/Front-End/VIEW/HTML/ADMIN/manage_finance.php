...
if (!isset($_SESSION['username']) && !isset($_SESSION['password'])) {
	...
	if(isset($_GET['Email'])){
		$Email=$_GET['Email'];
		$sql="DELETE FROM table_finance WHERE Email='$Email'";
		$query=mysqli_query($conn,$sql);

		if($query){
			$success="Cashier deleted from the system successfully";
			header("Location: manage_finance.php?success=$success");
			exit();
		}
	}
?>
	                      ...
	                      <th class="action">Action</th>
										...
										<tbody>
											...
											<tr>
												...
												<td><?php echo $date;?></td>
												<td class="action">
<a href="edit_finance.php?Email=<?php echo $row['Email'];?>" tooltip-placement="top" tooltip="Edit"><i class="fa fa-pencil"></i></a>
<a href="manage_finance.php?Email=<?php echo $row['Email']?>&del=delete" onClick="return confirm('Are you sure you want to delete Cashier <?php echo $row['Name']; ?>')"> <i class="fa fa-times fa fa-white"></i></a>
												</td>
											</tr>
											<?php $count=$count+1;
										}
