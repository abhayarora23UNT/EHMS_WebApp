...
if (strlen($Newpassword) < 6 ) {
  ...
  header("Location: ../../VIEW/HTML/PATIENT/password_reset.php?error=$error");
  exit();
}
...
$sql = "UPDATE table_patients2 SET Password = '$Password_hash' WHERE Email='$Useremail'";
$query = mysqli_query($conn,$sql);
if ($query) {
  $Userinput= strtolower($Useremail);
  $success="$Userinput, Password reset success please login with your new password.";
  header("Location: ../../VIEW/HTML/patient_login.php?success=$success");
}
}
}
else{
