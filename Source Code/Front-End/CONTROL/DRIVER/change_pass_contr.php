...
$sql="SELECT Password FROM table_patients2 WHERE Email='$Email'";
$query=mysqli_query($sql);
$Oldpassword=mysqli_fetch_row($query);

if (strlen($Newpassword) < 6 ) {
  $error = "New Password must be atleast 6 characters";
  header("Location:  ../../VIEW/HTML/PATIENT/change_password.php?error=$error");
  exit();
}
...
$sql = "UPDATE table_patients2 SET Password = '$password_hash' WHERE Email='$Email'";
$query = mysqli_query($conn,$sql);
if ($query) {
  $Userinput= strtolower($Email);
  $success="$Userinput, Your password has been updated successfully.";
  header("Location: ../../VIEW/HTML/PATIENT/change_password.php?success=$success");
}
}
}
else{
