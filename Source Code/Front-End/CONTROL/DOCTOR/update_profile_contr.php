...
if(isset($_POST["submit"])) {
  ...
  if(!$check) {
    ...
    header("Location: ../../VIEW/HTML/DOCTOR/update_profile.php?error=$error");
    exit();
    $uploadOk = 0;
  }
}
...
// if everything is ok, try to upload file
  ...
  $sql = "UPDATE table_doctor SET Avatar='$target_file' WHERE Email='$User'";
  $query=mysqli_query($conn,$sql) or die(mysqli_error($conn));
  if (move_uploaded_file($_FILES["Avatar"]["tmp_name"], $target_file)) {
    $success="Profile has been updated successfully";
    header("Location: ../../VIEW/HTML/DOCTOR/update_profile.php?success=$success");
    exit();
  } else {
    $error="Sorry, there was an error while uploading your profile";
    header("Location: ../../VIEW/HTML/DOCTOR/update_profile.php?error=$error");
