...
$sql="SELECT password FROM table_admin WHERE Name='$username'";
$query=mysqli_query($sql);
$Oldpassword=mysqli_fetch_row($query);

if (strlen($Newpassword) < 6 ) {
  $error = "New Password must be atleast 6 characters";
  header("Location:  ../../VIEW/HTML/ADMIN/change_password.php?error=$error");
  exit();
}
...
$sql = "UPDATE table_admin SET password = '$password_hash' WHERE Name='$username'";
$query = mysqli_query($conn,$sql);
if ($query) {
  $Userinput= strtolower($username);
  $success="$Userinput, Your password has been updated successfully.";
  header("Location: ../../VIEW/HTML/ADMIN/change_password.php?success=$success");
}
}
}
else{
