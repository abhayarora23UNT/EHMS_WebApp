...
exit();
  ...
  $error = "Patient '$Email'/'$Phone' already exist";
  header("Location:  ../../VIEW/HTML/ADMIN/add_patients.php?error=$error");
  exit();
}
  else{
//Insert
$sql = "INSERT INTO table_patients(FullName,Phone,Email,City,Address,DOB,Gender,Visit_date) VALUES('$Name','$Phone','$Email','$City','$Address','$Dob','$Gender','$Visit_date')";
$query = mysqli_query($conn,$sql);
if ($query) {
  $Userinput= strtolower($Name);
  $success="Patient $Userinput added successfuly";
  header("Location: ../../VIEW/HTML/ADMIN/add_patients.php?success=$success");
  exit();
}
}
}
