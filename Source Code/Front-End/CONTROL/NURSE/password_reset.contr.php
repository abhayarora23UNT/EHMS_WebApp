...
if (strlen($Newpassword) < 6 ) {
  ...
  header("Location: ../../VIEW/HTML/NURSE/password_reset.php?error=$error");
  exit();
}
...
$sql = "UPDATE table_nurse SET Password = '$Password_hash' WHERE Email='$Useremail'";
$query = mysqli_query($conn,$sql);
if ($query) {
  $Userinput= strtolower($Useremail);
  $success="$Userinput, Password reset success please login with your new password.";
  header("Location: ../../VIEW/HTML/nurse_login.php?success=$success");
}
}
}
else{
